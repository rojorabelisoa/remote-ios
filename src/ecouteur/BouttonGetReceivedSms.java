 ...
 * @author rojo
 */
public class BouttonGetReceivedSms  implements ActionListener {
    private IphoneSms s;

    public BouttonGetReceivedSms(IphoneSms aThis) {
        this.s=aThis;
    }
    @Override
    public void actionPerformed(ActionEvent e) {
        try {
            s.setPageActuelle("ReceivedSms");
            s.getRecherche().setText("");
            s.getTitre().setText("Liste des Sms Recus");
            s.getjTable1().removeAll();
            JTable tableau = TableauService.getSmsRecu();
