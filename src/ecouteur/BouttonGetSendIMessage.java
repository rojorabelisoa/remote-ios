 ...
 * @author rojo
 */
public class BouttonGetSendIMessage implements ActionListener {
    private IphoneSms s;
    
    public BouttonGetSendIMessage(IphoneSms aThis) {
        this.s=aThis;
    }
    @Override
    public void actionPerformed(ActionEvent e) {
        try {
            s.setPageActuelle("SendIMessage");
            s.getRecherche().setText("");
            s.getTitre().setText("Liste des iMessages Envoyées"); 
            s.getjTable1().removeAll();
            JTable tableau = TableauService.getIMessageEnvoyee();
